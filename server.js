...
function saveDatabase () {
  try {
  var saveDatabase = yaml.safeDump(database);
  fs.writeFile("database.yml", saveDatabase, (err) => {
    if(err) {
      console.error(err);
      return;
    };
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.username && database.users[requestComment.username] && database.articles[requestComment.articleId]) {
    const comment = {
    ...
    if(comment.id && comment.username && comment.body && comment.articleId && comment.commentIds && comment.upvotedBy && comment.downvotedBy) {
      response.status = 201;
    } else {
      response.status = 400;
    }
...
function updateComment (url, request) {
  ...
  if (!id || !requestComment) {
    ...
    database.comments[id].body = requestComment.body || savedComment.body;
...
function deleteComment (url, request) {
  ...
  if(savedComment) {
    ...
    const userArticleId = database.articles[savedComment.articleId].commentIds;
    userArticleId.splice(userArticleId.indexOf(id), 1);
